...
Environment:
- Python dependencies come from `requirements.txt` (simulation: the `ru551n/vunit` fork carrying the **VHDL-to-Python bridge** on the upstream `python_pkg` API — `add_python()` in `run.py`, `context vunit_lib.python_context` in VHDL) and `requirements-synth.txt` (netlist synthesis: released VUnit 4.7.1 — see that file for why the two cannot share one env). Only `hdl-modules` is a git submodule.
- Headless waveform recording (`--wave`) is not supported by the pinned VUnit and is not needed; diagnose from logs and Python tracebacks.
- The bridge compiles `libvunit_python_bridge.so` against CPython **on first use, at simulation time**, so any environment running simulations needs `Python.h` (`python3-dev`) and a C compiler, not merely at install time. CI gets the compiler from hdl-docker >= 1.3.0 and installs `python3-dev` itself.
