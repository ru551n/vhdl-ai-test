...
Environment:
- Python dependencies come from `requirements.txt` (simulation: the `ru551n/vunit` fork carrying the **VHDL-to-Python bridge**, enabled by `add_vhdl_builtins(python=True)` in `run.py`) and `requirements-synth.txt` (netlist synthesis: released VUnit 4.7.1 — see that file for why the two cannot share one env). Only `hdl-modules` is a git submodule.
- **Headless waveform recording is currently unavailable.** The bridge fork branch and the PR #1101 (`--wave`) fork branch diverged rather than one building on the other, so pinning the bridge means `--wave`, vunit-mcp's waveform support and peeper-mcp debugging are all off the table until the two are reconciled upstream. `modules/flash_model` needs the bridge, so this was a deliberate trade; diagnose from logs and Python tracebacks instead. If you need waveforms, reconciling the forks is the fix — do not silently repin, which would break the flash model.
- The bridge compiles `libvunit_python_bridge.so` against CPython **on first use, at simulation time**, so any environment running simulations needs `Python.h` and a C compiler (`cc`/`gcc`/`clang`), not merely at install time. That is why CI's simulate job installs `python3-dev` and `gcc`.
...
- `cnn_accel` constants are **generated, not hand-written**. Every register, HW property, opcode, flag bit and instruction-word byte offset is declared once in `modules/cnn_accel/cnn_accel_constants.py` and propagated into VHDL by `hdl-registers` (pinned in *both* requirements files): `module_cnn_accel.py`'s `registers_hook()` builds the register list, and the custom `cnn_accel_isa_generator.py` plugin emits the ISA package. Output lands in `modules/cnn_accel/regs_src/` and `regs_sim/`, both gitignored and regenerated by `run.py` and `build_fpga.py`. Edit the Python, never the generated `.vhd`. `cnn_accel_model.py` imports the same table, so the golden model's instruction encoding agrees with the RTL by construction rather than by comment.
