...
| channel-padded GIR | `02_depth_to_space_pad.txt` | every `depth_to_space` with a widenable producing conv has `out_channels` a whole multiple of the activation plane width, the added conv output channels are zero rows, and the true shape survives as `Tensor.logical_shape` |
| channel-permuted GIR | `03_depth_to_space_channels.txt` | every `depth_to_space` that has a permutable producing conv is `plane_major`; `interp` equivalence test |
| normalized GIR | `04_normalize.txt` | no identity clamp; still verifies |
| legalized GIR | `05_legalize_rescale.txt` | every rescale has `shift >= 15`, mult < 2^31, rounding ∈ accepted set |
| fused GIR | `06_fuse.txt` | every `fused_conv`'s epilogue ∈ target `epilogue`; `interp` equivalence test |
| HIR (mapped) | `07_hir.txt` | every op has a `unit` whose capability list admits its params; every constraint satisfied |
| HIR (scheduled) | `08_sched.txt` | `seq` total order; every dep has smaller `seq`; every read buffer written earlier or is input/const |
| HIR (planned) | `09_memplan.txt` | §8 verifier |
| program | `10_program.txt` (decoded descriptors) | `decode(encode(d)) == d`; `next_instr_addr` chain ends in `HALT`; all addresses ∈ planned buffers; reserved bytes 0 |
...
ACCEPTANCE: **MVP**: fixture compiled; `run.py` output == `interp` output == IREE output (byte-exact) for 3 seeds; decoded descriptors match HIR params; `10_program.txt` shows `CONV2D` + `HALT`. Note: passes only after H0 (until then the target refuses; test marked `xfail(reason="H0 pending")` so the gate is visible, not hidden).
