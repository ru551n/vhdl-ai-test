...
-- TDD testbench for modules/canny_gaussian3x3/src/canny_gaussian3x3.vhd,
...
entity tb_canny_gaussian3x3 is
  generic (
    -- Set per VUnit test config in module_canny_gaussian3x3.py: 0 for the
...
architecture tb of tb_canny_gaussian3x3 is
  ...
  constant c_data_width : positive := 9 * c_tap_width;
  ...
  signal m_axis_tdata : std_logic_vector(c_tap_width - 1 downto 0);
  ...
  begin
    if border = '1' then
      return std_logic_vector(to_unsigned(0, c_tap_width));
    end if;
    ...
    return std_logic_vector(to_unsigned((weighted_sum / 16) mod 256, c_tap_width));
  end function;
...
begin
  ...
  main : process
    ...
    procedure push_and_check(
      ...
      variable tdata : std_logic_vector(c_data_width - 1 downto 0);
    begin
      tdata := std_logic_vector(to_unsigned(tl, c_tap_width))
             & std_logic_vector(to_unsigned(tm, c_tap_width))
             & std_logic_vector(to_unsigned(tr, c_tap_width))
             & std_logic_vector(to_unsigned(ml, c_tap_width))
             & std_logic_vector(to_unsigned(mm, c_tap_width))
             & std_logic_vector(to_unsigned(mr, c_tap_width))
             & std_logic_vector(to_unsigned(bl, c_tap_width))
             & std_logic_vector(to_unsigned(bm, c_tap_width))
             & std_logic_vector(to_unsigned(br, c_tap_width));
    ...
    if run("test_random_data") then
      ...
      for word_idx in 0 to 299 loop
        for i in 0 to 8 loop
          t(i) := rnd.RandInt(0, 2 ** c_tap_width - 1);
        end loop;
