...
`modules/canny_gaussian3x3/doc/canny_gaussian3x3_req.md`. A pure
...
Single clock, synchronous active-low reset per the requirement's port
table. **Deviation/known limitation, confirmed against
`hdl-modules:modules/common/src/handshake_pipeline.vhd`:** the entity
has no reset port at all (only `clk` plus the input/output handshake
signals) — the same class of limitation already documented for
`canny_window3x3`'s reused `fifo.fifo_wrapper` (see
`modules/canny_window3x3/doc/canny_window3x3.md` "Clocking and reset").
`rst_n` is therefore accepted on this entity (per the requirement's
fixed port list) but cannot drive any reset inside the wrapped
instance; at elaboration `handshake_pipeline`'s own internal state
starts at its VHDL default (`wait_for_input_valid`, `output_valid =
'0'`, `input_ready = '1'`), which is the same externally-observable
"empty, ready" state a reset would produce. A single `rst_n` pulse
applied once before any beat is accepted (the pattern used by every
existing testbench in this repo) therefore behaves correctly; a
mid-stream reset would not flush an in-flight skid-buffered beat, which
is out of scope here for the same reason it was out of scope for
`canny_window3x3`'s line buffers.
...
- `gaussian_weighted_sum`: widened the 9 taps to a 12-bit accumulator
  (`c_tap_width + 4`, worst case `16*255=4080` fits) before summing the
...
- GHDL rejected `return (others => '0');` in `gaussian_weighted_sum`'s
  ...
  c_tap_width);` instead for the RED-phase stub, since GHDL cannot resolve
