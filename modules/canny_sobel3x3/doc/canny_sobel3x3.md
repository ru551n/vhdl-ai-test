...
Entity `canny_sobel3x3`, architecture `rtl`.
...
see "Dependencies") exposes no reset port. A `rst_n` pulse landing at the
...
`rst_n` once before any traffic, per `tb_axi_stream_join`/
...
- `common.handshake_pipeline` was evaluated for the one-cycle register but
  **not used** -- it exposes no reset port, and this module's
  `rst_n` must clear the output register per the requirement (see
  ...
  with an added synchronous `rst_n` clear.
...
- Independent `stall_probability_percent_mag`/`stall_probability_percent_dir`
  ...
  `module_canny_sobel3x3.py`: an all-zero full-throughput config, plus
