...
`modules/canny_sobel3x3/doc/canny_sobel3x3_req.md`. The two forks must
...
Ports: `clk`, `rst_n`, `s_axis_{tvalid,tready,tdata(71:0),tuser(1:0),tlast}`
...
Single clock, synchronous active-low reset. `rst_n` clears the one shared
...
s_axis_t* --> [combinational Gx/Gy/mag/dir + border forcing] --> reg_data_in (16 bits: mag(11) & dir(2) & tuser(2) & tlast(1))
                                                                        |
                                                     reg_valid/reg_data register (rst_n-clearable)
                                                                        |
...
- **`common.handshake_pipeline` — evaluated, not reused for the register.**
  ...
  section is explicit that `rst_n` must clear the output register(s), so
  ...
  internally — see that file — plus a synchronous `rst_n` clear that
...
- Dedicated VUnit unit test under `modules/canny_sobel3x3/test/`, using
...
- Independent, independently-randomized `stall_probability_percent_mag`/
  ...
  swept via `module_canny_sobel3x3.py`'s `add_vunit_config` — at least one
...
- **Known limitation (reset gap), same class as `canny_window3x3`'s FIFO
  ...
  hand-written `reg_valid`/`reg_data` register *is* cleared by `rst_n`,
  ...
  externally-visible level). However, if `rst_n` is pulsed at the exact
  ...
  `rst_n` once before any traffic, never mid-stream), and is not
