...
-- TDD testbench for modules/canny_sobel3x3/src/canny_sobel3x3.vhd, written
...
entity tb_canny_sobel3x3 is
  generic (
    -- Set per VUnit test config in module_canny_sobel3x3.py: all 0 for the
...
architecture tb of tb_canny_sobel3x3 is
  ...
  constant c_window_width : positive := 9 * c_tap_width;
  ...
  signal s_axis_tdata : std_logic_vector(c_window_width - 1 downto 0);
  ...
  signal m_axis_mag_tdata : std_logic_vector(c_mag_width - 1 downto 0);
  signal m_axis_mag_tuser : std_logic_vector(c_user_width - 1 downto 0);

  signal m_axis_dir_tvalid, m_axis_dir_tready, m_axis_dir_tlast : std_logic;
  signal m_axis_dir_tdata : std_logic_vector(c_dir_width - 1 downto 0);
  signal m_axis_dir_tuser : std_logic_vector(c_user_width - 1 downto 0);

  constant c_stall_config_in : stall_config_t := new_stall_config(
    stall_probability => real(stall_probability_percent_in) / 100.0,
    min_stall_cycles   => 1,
    max_stall_cycles   => 4
  );
  constant c_stall_config_mag : stall_config_t := new_stall_config(
    stall_probability => real(stall_probability_percent_mag) / 100.0,
    min_stall_cycles   => 1,
    max_stall_cycles   => 4
  );
  constant c_stall_config_dir : stall_config_t := new_stall_config(
  ...
  begin
    return std_logic_vector(to_unsigned(abs(gx) + abs(gy), c_mag_width));
  end function;
...
begin
  ...
  begin
    ...
    if rst_n = '1' and m_axis_mag_tvalid = '1' and m_axis_dir_tvalid = '1' then
  ...
  main : process
    ...
    procedure run_random_data_test(
      ...
      variable expected_mag : std_logic_vector(c_mag_width - 1 downto 0);
      variable expected_dir : std_logic_vector(c_dir_width - 1 downto 0);
    ...
    begin
      ...
      if check_throughput then
        ...
        check_relation(
          (now - start_time) < (num_beats + 10) * c_clk_period,
    ...
    procedure run_direction_sectors_test is
      ...
      constant c_cases : case_arr_t(0 to 5) := (
    ...
    begin
      for i in c_cases'range loop
        sof := to_sl(i = 0);
        is_last := to_sl(i = c_cases'high);

        push_axi_stream(
          net        => net,
          axi_stream => axi_master_in,
          tdata      => pack_window(
            c_cases(i).taps(0), c_cases(i).taps(1), c_cases(i).taps(2),
            c_cases(i).taps(3), c_cases(i).taps(4), c_cases(i).taps(5),
            c_cases(i).taps(6), c_cases(i).taps(7), c_cases(i).taps(8)
          ),
          tlast => is_last,
          tuser => c_cases(i).border & sof
        );
        ...
        -- Self-check: the hand-derived expected_dir in c_cases must agree
        -- with the independently-formulated calc_dir reference.
        assert (c_cases(i).border = '1') or (calc_dir(gx, gy) = c_cases(i).expected_dir)
          report "test bug: hand-derived direction for case " & to_string(i) &
            " disagrees with calc_dir reference"
          severity failure;

        expected_user := c_cases(i).border & sof;

        if c_cases(i).border = '1' then
          expected_mag := (others => '0');
        else
          expected_mag := calc_mag(gx, gy);
        end if;
        ...
        check_axi_stream(
          net        => net,
          axi_stream => axi_slave_dir,
          expected   => c_cases(i).expected_dir,
          tlast      => is_last,
          tuser      => expected_user,
          blocking   => false,
