...
raster-scan stream of `g_img_width x g_img_height` samples, with full elastic
...
`g_data_width`. See `modules/canny_window3x3/doc/canny_window3x3_req.md` and
`modules/canny_window3x3/doc/canny_window3x3_proposal.md`.

## Entity and architecture

Entity `canny_window3x3`, architecture `rtl`.

## Generics

| Name | Type | Meaning | Constraints |
|---|---|---|---|
| `g_img_width` | positive | frame width | > 0 |
| `g_img_height` | positive | frame height | > 0 |
| `g_data_width` | positive | per-sample bit width | > 0 |
| `g_user_width` | positive range 1 to 2 | width of `s_axis_tuser`; `1` = no incoming border bit (forced `'0'` internally), `2` = bit0 SOF, bit1 border | 1 or 2 |
...
| `s_axis_tdata` | in | std_logic_vector(g_data_width-1 downto 0) | input sample |
| `s_axis_tuser` | in | std_logic_vector(g_user_width-1 downto 0) | bit0=SOF, bit1 (only if width=2)=border |
...
| `m_axis_tdata` | out | std_logic_vector(9*g_data_width-1 downto 0) | 3x3 window, row-major MSB-to-LSB: `w_tl` top bits ... `w_br` bottom bits |
...
Single clock (`clk`), synchronous active-low reset (`rst_n`). `rst_n` clears
...
`fifo.fifo_wrapper` instances) are **not** cleared by `rst_n`, because that
...
empty), so a `rst_n` pulse applied once before any beat is ever accepted
(the pattern used by every existing testbench in this repo, e.g.
`tb_axi_stream_join`'s `rst_n_gen`) behaves correctly. A *mid-stream* reset
...
- Two elastic line-buffer FIFOs (`fifo.fifo_wrapper`, synchronous mode) each
  delay a live raster row by `g_img_width` accepted beats; a 3-tap
...
- The window *may* become valid once `g_img_width + 1` accepted input beats
  have occurred (the `primed` threshold); output is then 1:1 with accepted
  input beats while fresh input keeps arriving, plus a `g_img_width + 1`
  -beat tail-flush ("draining") of already-buffered windows once the frame's
  last input beat has been accepted. **Deviation from
  `doc/canny_window3x3_req.md`'s stated `2*g_img_width + 2`** -- see
  ...
  `g_img_width + 1` is the value this module actually needs for
...
- `m_axis_tuser(1)` = this module's own edge-of-frame test on
  ...
  `'0'` when `g_user_width=1`) -- the "growing border" dilate, see
  `doc/canny_arch.md` "Growing border (corrected, rev 2.2)".
- `m_axis_tdata`'s 9 `g_data_width`-wide slices are each tap's real stored
...
> last `g_img_width + 1` output windows of every frame -- already fully
> buffered and ready -- were never produced. See "New gotchas"/final report
> for the full incident writeup.

Latency: `g_img_width + 1` accepted input beats before the first valid
...
input keeps arriving, plus a `g_img_width + 1`-beat tail-flush at the end of
...
0-indexed accepted-beat count `t`, `W = g_img_width`: the "bottom"/live
...
accepted_beats >= g_img_width + 1`), so it is *sticky*: once true, it never
...
alongside a genuine new `fire`. But the very last `g_img_width + 1` output
...
been accepted, those last `g_img_width + 1` output windows would never be
...
`output_beat_count` (range `0 to g_img_width * g_img_height`, distinct from
...
- `all_input_received <= input_beat_count >= g_img_width * g_img_height`
- `all_output_sent <= output_beat_count >= g_img_width * g_img_height`
...
- `fifo.fifo_wrapper` (hdl-modules, `modules/fifo/src/fifo_wrapper.vhd`),
  ...
  `round_up_to_power_of_two(g_img_width + 1)` since `fifo.fifo_wrapper`'s
...
- Priming thresholds: `mid_row_active <= accepted_beats >= g_img_width`,
  `top_row_active <= accepted_beats >= 2*g_img_width`, `window_valid <=
  accepted_beats >= g_img_width + 1`. See "Timing/latency" above for the
  full derivation and the explicit, deliberate deviation from
  `doc/canny_window3x3_req.md`'s stated `2*g_img_width + 2` window-valid
...
- In-frame masking (`assemble_window` process): each of the 9 taps calls
  ...
  g_img_width, g_img_height)` to decide `m_axis_tdata`'s per-tap zero-mask
  and the tap's contribution (or not) to the border OR-reduction, per
  requirement.
- `border_in` (the incoming border bit, used only when `g_user_width = 2`)
  is derived via a pair of `if ... generate` statements
  (`gen_border_uw2`/`gen_border_uw1`), not a conditional signal assignment
  (`... when g_user_width = 2 else '0'`) -- see "New gotchas" in the final
  ...
  `g_user_width` crashes the `g_user_width=1` instance at simulation
...
- `stall_probability_percent` generic swept via `module_canny_window3x3.py`
...
- Border-dilate test with `g_user_width=2` and deliberately nonzero
