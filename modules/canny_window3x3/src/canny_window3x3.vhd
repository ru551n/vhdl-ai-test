...
entity canny_window3x3 is
  generic (
    ...
    g_user_width : positive range 1 to 2
  );
  port (
    clk   : in std_logic;
    rst_n : in std_logic;
    ...
    s_axis_tuser  : in  std_logic_vector(g_user_width - 1 downto 0);
    s_axis_tlast  : in  std_logic;

    m_axis_tvalid : out std_logic;
    m_axis_tready : in  std_logic;
    m_axis_tdata  : out std_logic_vector(9 * g_data_width - 1 downto 0);
    m_axis_tuser  : out std_logic_vector(1 downto 0);
    m_axis_tlast  : out std_logic
  );
end entity canny_window3x3;

architecture rtl of canny_window3x3 is
  ...
  constant c_lane_width : positive := g_data_width + 2;

  -- fifo.fifo_wrapper's synchronous-mode branch (fifo.vhd) requires a
  -- power-of-two memory depth; +1 gives headroom beyond g_img_width so the
  ...
  constant c_fifo_depth : positive := round_up_to_power_of_two(g_img_width + 1);

  function pack_lane(
    tdata  : std_logic_vector(g_data_width - 1 downto 0);
    border : std_logic;
    sof    : std_logic
  ...
  function lane_data(lane : std_logic_vector(c_lane_width - 1 downto 0))
    return std_logic_vector is
  begin
    return lane(c_lane_width - 1 downto 2);
  end function;

  function lane_border(lane : std_logic_vector(c_lane_width - 1 downto 0))
    return std_logic is
  begin
    return lane(1);
  end function;

  function lane_sof(lane : std_logic_vector(c_lane_width - 1 downto 0))
    return std_logic is
  begin
    return lane(0);
  end function;
  ...
  signal bottom_live : std_logic_vector(c_lane_width - 1 downto 0);
  signal bottom_mid_reg, bottom_left_reg : std_logic_vector(c_lane_width - 1 downto 0)
    := (others => '0');

  -- Middle row (out_row, the window center row).
  signal mid_live : std_logic_vector(c_lane_width - 1 downto 0);
  signal mid_mid_reg, mid_left_reg : std_logic_vector(c_lane_width - 1 downto 0)
    := (others => '0');

  -- Top row (out_row - 1).
  signal top_live : std_logic_vector(c_lane_width - 1 downto 0);
  signal top_mid_reg, top_left_reg : std_logic_vector(c_lane_width - 1 downto 0)
    := (others => '0');

  signal fifo1_write_ready, fifo1_read_valid : std_logic;
  signal fifo1_read_data : std_logic_vector(c_lane_width - 1 downto 0);

  signal fifo2_write_ready, fifo2_read_valid : std_logic;
  signal fifo2_read_data : std_logic_vector(c_lane_width - 1 downto 0);
  ...
  -- flip-flops.
  signal accepted_beats : natural range 0 to 2 * g_img_width + 2 := 0;
  ...
  signal input_beat_count : natural range 0 to g_img_width * g_img_height := 0;
  signal output_beat_count : natural range 0 to g_img_width * g_img_height := 0;

  signal mid_row_active, top_row_active : std_logic;

  -- 'primed': the fixed g_img_width + 1 accepted-beat priming threshold
  -- (see "Timing/latency" in doc/canny_window3x3.md). 'draining': all
  -- g_img_width * g_img_height input beats have been accepted but not
  -- every output window has been sent yet -- the tail-flush state needed
  -- because the last g_img_width + 1 output windows have no corresponding
  ...
  signal out_row : natural range 0 to g_img_height - 1 := 0;
  signal out_col : natural range 0 to g_img_width - 1 := 0;
...
begin
  ...
  -- here: g_user_width=1 instances have s_axis_tuser'range = (0 downto 0),
  -- so a runtime "s_axis_tuser(1) when g_user_width = 2 else '0'" still
  ...
  -- the g_user_width=1 instance.
  gen_border_uw2 : if g_user_width = 2 generate
    border_in <= s_axis_tuser(1);
  end generate gen_border_uw2;
  gen_border_uw1 : if g_user_width /= 2 generate
    border_in <= '0';
  ...
  --     fifo1, i.e. after g_img_width accepted beats -- the same instant
  ...
  --     g_img_width + 1 accepted beats (not 2*g_img_width + 2 -- that
  ...
  mid_row_active <= '1' when accepted_beats >= g_img_width else '0';
  top_row_active <= '1' when accepted_beats >= 2 * g_img_width else '0';
  primed         <= '1' when accepted_beats >= g_img_width + 1 else '0';

  -- Tail-flush ('draining') condition: once every g_img_width *
  -- g_img_height input beats has been accepted, no further *fresh* fire
  -- will ever happen for this frame, yet the last g_img_width + 1 output
  ...
  -- s_axis_tvalid, until all g_img_width * g_img_height outputs have been
  -- sent. Without this, the last g_img_width + 1 output windows would
  -- never be produced at all, since out_row/out_col would otherwise only
  -- ever advance in lock-step with a fresh input fire.
  all_input_received <= '1' when input_beat_count >= g_img_width * g_img_height else '0';
  all_output_sent     <= '1' when output_beat_count >= g_img_width * g_img_height else '0';
  ...
  -- g_img_width and 2*g_img_width vs. g_img_width + 1), so the mid/top
  ...
  begin
    if rising_edge(clk) then
      if rst_n = '0' then
        ...
        if fire then
          if accepted_beats < 2 * g_img_width + 2 then
            accepted_beats <= accepted_beats + 1;
          end if;
          if input_beat_count < g_img_width * g_img_height then
            input_beat_count <= input_beat_count + 1;
          end if;
        end if;
        if window_valid and m_axis_tready then
          if output_beat_count < g_img_width * g_img_height then
  ...
  begin
    if rising_edge(clk) then
      if rst_n = '0' then
        ...
        if out_col = g_img_width - 1 then
          out_col <= 0;
          if out_row = g_img_height - 1 then
            out_row <= 0;
          else
            out_row <= out_row + 1;
          end if;
  ...
  -- the last g_img_width + 1 output windows would repeat stale mid/top
  ...
  ------------------------------------------------------------------------------
  -- Line buffer 1: delays the live (bottom) row by g_img_width accepted
  ...
  ------------------------------------------------------------------------------
  -- Line buffer 2: delays the middle row by another g_img_width accepted
  ...
  m_axis_tlast <= '1' when out_col = g_img_width - 1 else '0';
  ...
  assemble_window : process(
    ...
    variable lane : std_logic_vector(c_lane_width - 1 downto 0);
  ...
  begin
    ...
    for row in tap_row_t'left to tap_row_t'right loop
      for col in tap_col_t'left to tap_col_t'right loop
        ...
        in_frame := tap_in_frame(
          out_row, out_col, row_off(row), col_off(col), g_img_width, g_img_height
        );

        if in_frame then
          m_axis_tdata(
            (9 - (3 * tap_row_t'pos(row) + tap_col_t'pos(col))) * g_data_width - 1
            downto (8 - (3 * tap_row_t'pos(row) + tap_col_t'pos(col))) * g_data_width
          ) <= lane_data(lane);
    ...
    if out_row = 0 or out_row = g_img_height - 1
      or out_col = 0 or out_col = g_img_width - 1
    then
      edge_here := '1';
    else
      edge_here := '0';
