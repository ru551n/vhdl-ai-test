...
`modules/canny_nms/doc/canny_nms_req.md`: keep the center magnitude of a
...
No generics. Ports: `clk`, `rst_n`, `s_axis_{tvalid,tready,tdata,tuser,tlast}`,
...
Single clock, synchronous active-low reset. `common.handshake_pipeline`
...
entity ports: `clk`, `input_*`, `output_*`, no `rst`/`rst_n`). To satisfy
the requirement's "synchronous active-low reset clears the
...
`rst_n`: `m_axis_tvalid <= pipeline_output_valid and rst_n`. This makes the
...
handshake_pipeline.output_valid and rst_n --> m_axis_tvalid
```
...
- Reset asserted mid-stream: `m_axis_tvalid` forced low via the `rst_n`
...
- Dedicated VUnit unit test under `modules/canny_nms/test/`, using VUnit's
...
- `stall_probability_percent` generic, swept via `module_canny_nms.py` (0
