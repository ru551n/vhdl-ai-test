...
Entity `canny_nms`, architecture `rtl`.
...
Single clock (`clk`), synchronous active-low reset (`rst_n`). Reused
`common.handshake_pipeline` has no reset port; this module gates the
externally visible `m_axis_tvalid` with `rst_n`
(`m_axis_tvalid <= pipeline_output_valid and rst_n`) so the output reads
...
- `common.handshake_pipeline` has no reset port at all (confirmed against
  its source). `m_axis_tvalid` is gated with `rst_n`
  (`m_axis_tvalid <= pipeline_output_valid and rst_n`) so the
