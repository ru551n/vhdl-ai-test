...
-- TDD testbench for modules/canny_nms/src/canny_nms.vhd, written before the
...
entity tb_canny_nms is
  generic (
    -- Set per VUnit test config in module_canny_nms.py: 0 for the
...
architecture tb of tb_canny_nms is
  ...
  constant c_data_width : positive := 9 * c_mag_width + 2;
  ...
  signal m_axis_tdata : std_logic_vector(c_mag_width - 1 downto 0);
  ...
  type taps9_t is array(0 to 8) of std_logic_vector(c_mag_width - 1 downto 0);
  ...
  function compute_expected(
    ...
    variable mm, neighbor_a, neighbor_b : unsigned(c_mag_width - 1 downto 0);
  begin
    mm := unsigned(taps(4));
    ...
    if border = '1' then
      return (c_mag_width - 1 downto 0 => '0');
...
begin
  ...
  main : process
    ...
    procedure push_and_check(
      ...
      variable expected_data : std_logic_vector(c_mag_width - 1 downto 0);
    begin
      in_user := border & sof;
    ...
    begin
      ...
      for i in sectors'range loop
        ...
        for t in taps'range loop
          taps(t) := std_logic_vector(to_unsigned(10 + t, c_mag_width));
        end loop;

        -- Case 1: local maximum -- center strictly greater than both
        -- direction-relevant neighbors -- must be kept.
        taps(4) := std_logic_vector(to_unsigned(200, c_mag_width));
        push_and_check(
        ...
        taps(4) := std_logic_vector(to_unsigned(5, c_mag_width));
        push_and_check(
        ...
        for t in taps'range loop
          taps(t) := std_logic_vector(to_unsigned(50, c_mag_width));
        end loop;
        push_and_check(
      ...
      for beat in 0 to num_beats - 1 loop
        for t in taps'range loop
          taps(t) := std_logic_vector(
            to_unsigned(rnd.RandInt(0, 2 ** c_mag_width - 1), c_mag_width)
          );
        end loop;
    ...
    begin
      for beat in 0 to 7 loop
        ...
        taps(4) := std_logic_vector(to_unsigned(2 ** c_mag_width - 1, c_mag_width));
      ...
      for t in taps'range loop
        taps(t) := std_logic_vector(to_unsigned(1, c_mag_width));
      end loop;
      taps(4) := std_logic_vector(to_unsigned(2 ** c_mag_width - 1, c_mag_width));
