...
| `cfg_pad_top`, `cfg_pad_bottom`, `cfg_pad_left`, `cfg_pad_right` | in | `std_ulogic_vector(7 downto 0)` | Padding on each side, latched at `start`. |
| `cfg_pad_value` | in | `std_ulogic_vector(7 downto 0)` | **ISA v2.1**, latched at `start`: the signed int8 value every padded tap takes — the input tensor's quantization zero-point, not necessarily 0. Defaults to 0, which is the pre-v2.1 zero-fill exactly. `cnn_accel_top` drives it from the descriptor's `pad_value` on BOTH instances (the pool one directly, the conv one through `cnn_accel_conv_core`'s port of the same name), each from its own `cnn_accel_cmd_proc` output, so a program may pad a convolution and a following pool with different values. Implemented as the fill value of the tap-assembly register's per-window clear, so taps that are never written — out-of-frame *or* beyond the runtime `K_h*K_w` — read back as the pad value. |
...
g_tile_channels` windows, one per input-channel tile (substituting
`cfg_pad_value` for any tap that falls in the padding region, per
`cfg_pad_*`), as `T`
