...
architecture a of cnn_accel_top is
  ...
  signal read_m2s_vec : axi_read_m2s_vec_t(0 to c_num_read_inputs - 1) := (others => axi_read_m2s_init);
  ...
  signal instr_ar_m2s : axi_m2s_a_t := axi_m2s_a_init;
  signal load_ar_m2s : axi_m2s_a_t := axi_m2s_a_init;
  signal wgt_ar_m2s : axi_m2s_a_t := axi_m2s_a_init;
  signal instr_ar_s2m : axi_s2m_a_t;
  signal load_ar_s2m : axi_s2m_a_t;
  signal wgt_ar_s2m : axi_s2m_a_t;
  signal instr_r_m2s : axi_m2s_r_t := axi_m2s_r_init;
  signal load_r_m2s : axi_m2s_r_t := axi_m2s_r_init;
  signal wgt_r_m2s : axi_m2s_r_t := axi_m2s_r_init;
  signal instr_r_s2m : axi_s2m_r_t;
  signal load_r_s2m : axi_s2m_r_t;
  signal wgt_r_s2m : axi_s2m_r_t;
  ...
  constant c_req_init : dma_req_m2s_t := (valid => '0', req => (addr => (others => '0'), length => (others => '0')));
  ...
  signal conv_cfg_bias_en : std_ulogic;
  signal conv_cfg_requant_en : std_ulogic;
  signal conv_cfg_relu_en : std_ulogic;
  ...
  signal conv_fill_start : std_ulogic := '0';
  signal conv_fill_is_bias : std_ulogic := '0';
  signal conv_fill_is_scale : std_ulogic := '0';
  ...
  signal pool_window_m2s : window_m2s_t(data(0 to window_data_length(g_max_pool_kernel_size, g_tile_channels) - 1));
  ...
  signal pool_accum_m2s : accum_m2s_t(data(0 to g_tile_channels - 1)(g_accum_width - 1 downto 0));
  ...
  signal ew_src0_addr : unsigned(31 downto 0);
  signal ew_src1_addr : unsigned(31 downto 0);
  signal ew_dst_addr : unsigned(31 downto 0);
  signal ew_lut_addr : unsigned(31 downto 0);
  signal ew_xfer_bytes : unsigned(31 downto 0);
  signal ew_in_width : unsigned(15 downto 0);
  signal ew_in_height : unsigned(15 downto 0);
  signal ew_in_channels : unsigned(15 downto 0);
...
begin
  ...
  assert g_tile_channels = cnn_accel_constant_activation_plane_channels
    report "cnn_accel_top: g_tile_channels ("
           & positive'image(g_tile_channels)
  ...
  assert g_pe_rows = cnn_accel_constant_activation_plane_channels
    report "cnn_accel_top: g_pe_rows ("
           & positive'image(g_pe_rows)
           & ") must equal the generated activation-plane channel count ("
           & integer'image(cnn_accel_constant_activation_plane_channels)
  ...
  assert g_max_pool_kernel_size >= g_max_kernel_size
    report "cnn_accel_top: g_max_pool_kernel_size ("
           & positive'image(g_max_pool_kernel_size)
           & ") must be >= g_max_kernel_size ("
  ...
  assert g_axi_data_width = 8 * g_tile_channels
    report "cnn_accel_top: g_axi_data_width ("
  ...
  begin
    input_m2s <= tm_r0_raw_m2s when r = 0 else
                 tm_r1_raw_m2s;
    output_ready <= tm_r0_s2m.ready when r = 0 else
                    tm_r1_s2m.ready;
  ...
  end generate tm_read_pipeline_gen;
  ...
  begin
    input_m2s <= tm_w0_m2s when w = 0 else
                 tm_w1_m2s;
    output_ready <= tm_w0_piped_s2m.ready when w = 0 else
                    tm_w1_piped_s2m.ready;
  ...
  end generate tm_write_pipeline_gen;
    ...
    if rising_edge(clk) then
      for lane in pool_lane_ready_vec'range loop
        ...
        assert pool_lane_ready_vec(lane) = pool_lane_ready_vec(0)
          report "cnn_accel_top: pool lane "
                 & natural'image(lane)
                 & " disagrees with "
  ...
  begin
    ...
    for lane in 0 to g_tile_channels - 1 loop
      ...
      pool_max_m2s.data(8 * lane + 7 downto 8 * lane) <= pool_lane_max_m2s(lane).data(7 downto 0);
    end loop;
  ...
  begin
    ...
    for lane in 0 to g_tile_channels - 1 loop
      ...
      pool_accum_m2s.data(lane) <= signed(pool_lane_avgsum_m2s(lane).data(g_accum_width - 1 downto 0));
    end loop;
  ...
  pool_is_avg <= '1' when pool_cfg_opcode = c_opcode_pool_avg else
                 '0';

  pool_out_m2s <= pool_avg_m2s when pool_is_avg = '1' else
                  pool_max_m2s;
