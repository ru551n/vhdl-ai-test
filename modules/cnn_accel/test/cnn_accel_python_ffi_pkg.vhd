...
package cnn_accel_python_ffi_pkg is
  ...
  procedure ffi_write_bytes (memory : memory_t; function_name : string; base_addr : natural; num_bytes : natural);
  ...
  impure function ffi_export_bytes (memory : memory_t; base_addr : natural; num_bytes : natural) return integer_array_t;
...
package body cnn_accel_python_ffi_pkg is
  ...
  procedure ffi_write_bytes (memory : memory_t; function_name : string; base_addr : natural; num_bytes : natural) is
  ...
  begin
    ...
    check_equal(
      length(data),
      num_bytes,
      "ffi_write_bytes: call("""
      & function_name
      & """) returned "
      & to_string(length(data))
      & " bytes, expected "
      & to_string(num_bytes)
    );
    ...
    for i in 0 to num_bytes - 1 loop
      ...
      write_word(memory => memory, address => base_addr + i, word => std_logic_vector(to_unsigned(get(data, i), 8)));
    end loop;
  ...
  begin
    ...
    for i in 0 to num_bytes - 1 loop
      ...
      set(data, i, to_integer(u_unsigned(read_word(memory => memory, address => base_addr + i, bytes_per_word => 1))));
    end loop;
