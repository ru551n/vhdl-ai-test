...
architecture tb of tb_cnn_accel_pe_array_from_vectors is
  ...
  signal cfg_kernel_h : std_ulogic_vector(7 downto 0) := std_ulogic_vector(to_unsigned(c_max_kernel, 8));
  signal cfg_kernel_w : std_ulogic_vector(7 downto 0) := std_ulogic_vector(to_unsigned(c_max_kernel, 8));
  ...
  begin
    ...
    if run("test_cross_language_bitexact") then
      ...
      for pixel in 0 to c_num_pixels - 1 loop
        ...
        check_equal(
          pack_actual(m_accum_m2s.data),
          pack_expected(expected),
          "m_accum data mismatch (Python-packed weights vs. real RTL) at pixel " & integer'image(pixel)
        );
        check_equal(
          m_accum_m2s.last,
          to_sl(pixel = c_num_pixels - 1),
