...
package cnn_accel_tb_util_pkg is
  ...
  function axi_response_latency (stall_probability_percent : natural; clk_period : time) return time;
  ...
  -- site.
  function describe_status (status_slv : register_t; status : cnn_accel_status_t) return string;
...
package body cnn_accel_tb_util_pkg is
  ...
  function axi_response_latency (stall_probability_percent : natural; clk_period : time) return time is
  begin
  ...
  function describe_status (status_slv : register_t; status : cnn_accel_status_t) return string is
  begin

    return "STATUS=0x"
           & to_hex(u_unsigned(status_slv), 8)
           & " (busy="
           ...
           & ", err_code="
           & to_dec(status.err_code)
           & ", err_pc_low="
           & to_dec(status.err_pc_low)
           & ")";
  end function;
