...
| S2 | **Default stays 8 (30 fps).** A 16-row configuration is added as a netlist build (both backends) and a testbench config so the scale-up path is proven in CI, not switched on. |
