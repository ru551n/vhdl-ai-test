...
`modules/canny_hysteresis/doc/canny_hysteresis_req.md` and
`modules/canny_hysteresis/doc/canny_hysteresis_proposal.md`.

## Entity and architecture

Entity `canny_hysteresis`, architecture `rtl`.
...
Single clock (`clk`), synchronous active-low reset (`rst_n`). `rst_n`
...
- Reset-gating around `common.handshake_pipeline` (which has no reset port
  of its own): `pipeline_input_valid <= s_axis_tvalid and rst_n`;
  `pipeline_output_ready <= m_axis_tready or not rst_n` (force-drains the
  pipeline within its own latency while `rst_n = '0'`); `s_axis_tready <=
  pipeline_input_ready and rst_n`; `m_axis_tvalid <= pipeline_output_valid
  and rst_n`. No deviation from the proposal's design was needed.
