...
2.1)". See `modules/canny_hysteresis/doc/canny_hysteresis_req.md` for the
full requirement.
...
port at all** — only `clk`. To still honor this module's own `rst_n`
...
- `input_valid` (into the pipeline) is gated with `rst_n` so no new beat is
  presented to the pipeline while in reset.
- `output_ready` (into the pipeline, i.e. what would normally just be
  `m_axis_tready`) is forced to `'1'` while `rst_n='0'` (`m_axis_tready or
  not rst_n`) — this force-drains any state the pipeline's internal skid
  ...
  reasonable length (every testbench in this repo holds `rst_n='0'` for
  several cycles before the test body starts, e.g. `tb_axi_stream_join`'s
  `rst_n_gen`).
- `m_axis_tvalid` is additionally masked with `rst_n`
  (`pipeline_output_valid and rst_n`) so no beat — stale or otherwise — is
  ever visible on `m_axis` while reset is asserted, even during the (at
  most 2-cycle) internal drain window above.
- `s_axis_tready` is masked with `rst_n` the same way, so the upstream
...
s_axis_tdata(17:0), s_axis_tuser(1:0) --> compute_edge (combinational fn)
                          ...
                          input_valid <= s_axis_tvalid and rst_n
                          input_last  <= s_axis_tlast
                          output_ready <= m_axis_tready or not rst_n
                                            |
                                            v
              ...
              m_axis_tlast <= output_last; m_axis_tvalid <= output_valid and rst_n
```
...
- `s_axis_tdata`/`m_axis_tdata`: `std_logic_vector`, opaque packed
  ...
  unlike `canny_window3x3`'s `g_user_width`-conditioned slicing).
...
- Dedicated VUnit unit test under `modules/canny_hysteresis/test/`, using
...
- `stall_probability_percent : natural` generic, swept via
  `module_canny_hysteresis.py` (0 for the dedicated full-throughput test,
...
reset-gating assigns `pipeline_input_valid <= s_axis_tvalid and rst_n`,
`pipeline_output_ready <= m_axis_tready or not rst_n`, `s_axis_tready <=
pipeline_input_ready and rst_n`, `m_axis_tvalid <= pipeline_output_valid and
rst_n`. No deviation from this document's design was needed for the RTL
itself.
