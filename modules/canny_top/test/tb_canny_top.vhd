...
-- IP-level integration testbench for modules/canny_top/src/canny_top.vhd,
...
--
-- module_canny_top.py's pre_config() writes a random stimulus frame
...
-- expected pixel/edge value itself. module_canny_top.py's post_check()
...
entity tb_canny_top is
  generic (
    -- Set per VUnit test config in module_canny_top.py.
...
begin
  ...
  main : process
    ...
    variable rx_data  : std_logic_vector(c_data_width - 1 downto 0);
    variable rx_tlast : std_logic;
    variable rx_tkeep : std_logic_vector(c_data_width / 8 - 1 downto 0);
    variable rx_tstrb : std_logic_vector(c_data_width / 8 - 1 downto 0);
  ...
  begin
    ...
    if run("test_full_pipeline") then
      ...
      assert width(stim_arr) = g_img_width and height(stim_arr) = g_img_height
        report "tb_canny_top: stimulus.csv shape does not match g_img_width/g_img_height"
        severity failure;

      result_arr := new_2d(
        width => g_img_width, height => g_img_height, bit_width => 8, is_signed => false
      );
      ...
      for row in 0 to g_img_height - 1 loop
        for col in 0 to g_img_width - 1 loop
          pixel := get(stim_arr, col, row);
          push_axi_stream(
            net        => net,
            axi_stream => axi_master,
            tdata      => std_logic_vector(to_unsigned(pixel, c_data_width)),
            tlast      => to_sl(col = g_img_width - 1),
      ...
      for row in 0 to g_img_height - 1 loop
        for col in 0 to g_img_width - 1 loop
          ...
          check_equal(
            rx_tlast, to_sl(col = g_img_width - 1),
