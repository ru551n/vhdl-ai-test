...
rationale — this file exists to give `canny_top` a `modules/canny_top/doc/`
...
- `W1..W4` are 4 instances of `canny_window3x3`, generic-mapped with the
  `g_data_width` appropriate to their position (8, 8, 11, 2) and
  `g_user_width` = 1 for `W1` only (fed directly from `s_axis_tuser`, 1
...
- `sobel`'s two forks (`m_axis_mag_*`, `m_axis_dir_*`) feed `W3` and
  ...
  `2*g_img_width+2` cycles) without ever stalling the `sobel` output —
  the exact depth is a `vhfill`-time sizing decision, not fixed here.
- `axi_stream_join`'s two inputs are `W3`'s output (`g_data_width_a=99`)
  and `axi_stream_fifo`'s output (`g_data_width_b=2`); its single output
  feeds `nms`.
- `hysteresis`'s output connects directly to `m_axis_*` — no further
  ...
  `modules/canny_hysteresis/doc/canny_hysteresis_req.md`).
