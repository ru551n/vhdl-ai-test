...
-- TDD testbench for modules/canny/src/canny_window3x3.vhd, written
...
--
-- Two independent DUT instances are used since user_width is a
-- generic (affects s_axis_tuser's port width, so it can not be swept at
-- run time on a single instance): dut_uw1 (user_width => 1, no incoming
-- border bit at all) is used for test_random_data/test_full_throughput;
-- dut_uw2 (user_width => 2) is used for test_border_dilate.
...
architecture tb of tb_canny_window3x3 is
  ...
  -- dut_uw1: user_width => 1.
  constant img_width1  : positive := 8;
  constant img_height1 : positive := 6;

  -- dut_uw2: user_width => 2.
  constant img_width2  : positive := 6;
  constant img_height2 : positive := 5;
  ...
  type frame1_data_t is array(0 to img_height1 - 1, 0 to img_width1 - 1)
    of std_logic_vector(data_width - 1 downto 0);

  type frame2_data_t is array(0 to img_height2 - 1, 0 to img_width2 - 1)
    of std_logic_vector(data_width - 1 downto 0);
  type frame2_border_t is array(0 to img_height2 - 1, 0 to img_width2 - 1) of std_logic;
...
begin
  ...
  main : process
    ...
    -- (user_width => 1, so no incoming border bit exists at all), and
    ...
    procedure run_random_data_test(check_throughput : boolean := false) is
      ...
      variable expected_data : std_logic_vector(9 * data_width - 1 downto 0);
    ...
    begin
      ...
      for row in 0 to img_height1 - 1 loop
        for col in 0 to img_width1 - 1 loop
          frame(row, col) := std_logic_vector(
            to_unsigned(rnd.RandInt(0, 2 ** data_width - 1), data_width)
          );
        end loop;
      end loop;
      ...
      for row in 0 to img_height1 - 1 loop
        for col in 0 to img_width1 - 1 loop
          ...
          is_last := to_sl(col = img_width1 - 1);
          in_user(0) := sof;
          ...
          for row_off in -1 to 1 loop
            for col_off in -1 to 1 loop
              ...
              if in_frame(tap_row, tap_col, img_width1, img_height1) then
                expected_data(
                  (9 - byte_idx) * data_width - 1 downto (8 - byte_idx) * data_width
          ...
          -- user_width => 1: no incoming border bit exists, so the
          -- output border bit is exactly this module's own edge-of-frame
          -- test (no dilate contribution possible).
          expected_user := to_sl(
            row = 0 or row = img_height1 - 1 or col = 0 or col = img_width1 - 1
          ) & sof;
      ...
      if check_throughput then
        ...
        -- the full frame plus the fixed 2*img_width+2 fill latency, plus
        -- a small margin for reset/startup.
        check_relation(
          (now - start_time)
            < (img_width1 * img_height1 + 2 * img_width1 + 2 + 10) * clk_period,
          "canny_window3x3 did not sustain full throughput at zero stall"
        );
      end if;
    end procedure;

    -- Directed border-dilate test on dut_uw2 (user_width => 2): a single
    ...
    procedure run_border_dilate_test is
      ...
      constant border_row : natural := 2;
      constant border_col : natural := 3;
    begin
      for row in 0 to img_height2 - 1 loop
        for col in 0 to img_width2 - 1 loop
          frame(row, col) := std_logic_vector(
            to_unsigned(rnd.RandInt(0, 2 ** data_width - 1), data_width)
          );
          border(row, col) := to_sl(row = border_row and col = border_col);
        end loop;
      end loop;
      ...
      for row in 0 to img_height2 - 1 loop
        for col in 0 to img_width2 - 1 loop
          sof := to_sl(row = 0 and col = 0);
          is_last := to_sl(col = img_width2 - 1);
          ...
          for row_off in -1 to 1 loop
            for col_off in -1 to 1 loop
              ...
              if in_frame(tap_row, tap_col, img_width2, img_height2) then
                expected_data(
                  (9 - byte_idx) * data_width - 1 downto (8 - byte_idx) * data_width
                ) := frame(tap_row, tap_col);
          ...
          self_edge := to_sl(
            row = 0 or row = img_height2 - 1 or col = 0 or col = img_width2 - 1
          );
