...
-- TDD testbench for modules/canny/src/canny_threshold.vhd, written
...
architecture tb of tb_canny_threshold is
  ...
  constant data_width : positive := 11;
  constant user_width : positive := 2;

  constant thresh_low  : natural := 100;
  constant thresh_high : natural := 200;
  ...
  -- thresh_high; "01" (weak) at/above thresh_low; else "00" (none).
  ...
  begin
    if border = '1' then
      return "00";
    elsif magnitude >= thresh_high then
      return "10";
    elsif magnitude >= thresh_low then
      return "01";
    else
      return "00";
    end if;
...
begin
  ...
  main : process
    ...
    begin
      push_axi_stream(
        ...
        tdata      => std_logic_vector(to_unsigned(magnitude, data_width)),
        tlast      => is_last,
        tuser      => border & sof
      );
  ...
  begin
    ...
    if run("test_boundary_values") then
      -- Directed: exactly thresh_low-1, thresh_low, thresh_high-1,
      -- thresh_high, non-border.
      push_and_check(thresh_low - 1, '0', '1', '0', "thresh_low-1");
      push_and_check(thresh_low, '0', '0', '0', "thresh_low");
      push_and_check(thresh_high - 1, '0', '0', '0', "thresh_high-1");
      push_and_check(thresh_high, '0', '0', '1', "thresh_high");

    elsif run("test_random_data") then
      -- Random magnitude across the full 0..2047 (11-bit) input range,
      -- random border bit.
      for word_idx in 0 to 299 loop
        magnitude := rnd.RandInt(0, 2 ** data_width - 1);
      ...
      push_and_check(thresh_low + 10, '1', '1', '0', "border forces none (weak magnitude)");
      push_and_check(thresh_high + 10, '1', '0', '0', "border forces none (strong magnitude)");
  ...
  checker_proc : process
    ...
    variable got_tuser : std_logic_vector(user_width - 1 downto 0);

    variable expected_tdata : std_logic_vector(1 downto 0);
    variable expected_tlast : std_logic;
    variable expected_tuser : std_logic_vector(user_width - 1 downto 0);
  begin
    loop
      pop_axi_stream(
        net        => net,
