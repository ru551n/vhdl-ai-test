...
`modules/canny/doc/canny_window3x3_req.md`. Full elastic
backpressure on both sides. Reused 4x in the canny pipeline with
different `data_width`. Emits, for every accepted input beat (once
...
Generics: `img_width : positive`, `img_height : positive`,
`data_width : positive`, `user_width : positive range 1 to 2`.
...
table. `s_axis_tuser` is `user_width` bits (bit0=SOF, bit1=border only
present when `user_width=2`); `m_axis_tuser` is always 2 bits
(bit0=SOF, bit1=border); `m_axis_tdata` is `9*data_width` bits, packed
row-major MSB-to-LSB (`w_tl` at the top, `w_br` at the bottom).

## Clock/reset behavior

Single clock, synchronous active-high reset. Reset clears all internal
line-buffer FIFOs (via their own `reset`-less design — see
...
s_axis ---+--> bottom-row 3-tap shift --+--> fifo1 (line buf, depth=W) --> mid-row 3-tap shift --+--> fifo2 (line buf, depth=W) --> top-row 3-tap shift
          |    (right=live, mid, left) |         (delay = img_width)   (right=live, mid, left) |         (delay = img_width)   (right=live, mid, left)
          |                            |                                                          |
...
- `fifo1`/`fifo2` payload width is `data_width + 3`: `tdata`, a
  `border` bit (forced `'0'` internally when `user_width=1`, since no
...
- Depth of each `fifo.fifo_wrapper` instance is
  `round_up_to_power_of_two(img_width + 1)` (hdl-modules `math_pkg`),
  not `img_width` itself — see "Corner cases" below on why the `+1`
  ...
  needs a *power-of-two* depth regardless of `img_width`'s own value.
...
- Priming is tracked by a single saturating `accepted_beats` counter and
  ...
  *not* `m_axis_tvalid`). Per the requirement, output becomes available
  once `2*img_width + 2` accepted input beats have occurred — **the
...
- Output-side `out_row`/`out_col` counters (0-based) are pure functions
  ...
  widths"), reset by `reset`, not by `s_axis_tuser(0)`/`tlast` directly;
...
- `m_axis_tdata`'s 9 slices: each tap's `data_width` bits are the tap's
...
- `out_row`/`out_col`: plain `natural range 0 to img_height-1` /
  `0 to img_width-1` — small enough range that `natural` (rather than
...
- `accepted_beats`: `natural range 0 to 2*img_width+1` (saturating) —
...
- FIFO payload width `data_width + 3`: `std_logic_vector`, packed via
...
Latency: `2*img_width + 2` accepted input beats before the first valid
...
implementation's actual latency is `img_width + 1` accepted input
beats, not `2*img_width + 2` — that was already flagged as a deliberate
...
last `img_width + 1` output beats of every frame are produced during a
...
- FIFO depth vs. `img_width`: `fifo.fifo_wrapper`/`fifo.fifo` requires
  a *power-of-two* memory depth (`fifo.vhd`'s own elaboration-time
  assertion), which `img_width` need not be. Depth is chosen as
  `round_up_to_power_of_two(img_width + 1)` — strictly more than
  `img_width` even when `img_width` is itself already a power of
...
- `user_width = 1`: no incoming border bit exists on `s_axis_tuser` at
...
- Multiple back-to-back frames on the same instance: line buffers are
  never drained/reset between frames (only `reset` clears them); the
  `out_row`/`out_col` counters wrap naturally at `img_height`/
  `img_width` boundaries, so a second frame's first output beat lands
...
- `stall_probability_percent` generic, swept via
  ...
  `stall_config`.
- `test_random_data`: small frame (`user_width=1`, no incoming border
...
- `test_full_throughput`: larger frame, zero stall on all sides,
  ...
  small margin.
- `test_border_dilate`: `user_width=2`, small frame, deliberately
...
The `2*img_width + 2` accepted-beat priming threshold and the exact
...
- **In-frame masking**: filled in as anticipated, calling
  ...
  img_width, img_height)` per tap.
- **Priming thresholds**: `mid_row_active`'s `img_width` threshold was
  already correct. `top_row_active` needed `2*img_width` (not
  `img_width`). `window_valid` needed `img_width + 1` — **not**
  `2*img_width + 2` as this document and `doc/canny_window3x3_req.md`
  ...
  exactly `img_width + 1` — using `2*img_width + 2` instead would
  ...
  `2*img_width + 2` remains correct as the point at which the single
...
- **Also found during the RED confirmation run** (not one of the two
  ...
  <= s_axis_tuser(1) when user_width = 2 else '0';` crashed at
  simulation start for the `user_width=1` instance with an out-of-bounds
...
- **Also found during GREEN**: the testbench's `check_axi_stream` calls
  ...
  `img_width + 1` accepted input beats before any output appears.
...
- **Missing tail-flush ("draining") bug**: gating output on genuinely
  ...
  anticipate: the frame's last `img_width + 1` output windows have no
...
`img_width`, `top_row_active` at `2*img_width`, and `primed`
(formerly `window_valid`) at `img_width + 1` — re-verified
