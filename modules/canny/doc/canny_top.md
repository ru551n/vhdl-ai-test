...
`modules/canny/doc/canny_top_req.md` for the requirement and
`modules/canny/doc/canny_top_proposal.md` for the implementation
rationale (including deviations discovered/resolved during wiring: flat
ports throughout rather than records, no `img_width`/`img_height` on
...
`modules/canny/src/canny_top.vhd` for exact port maps.

## Status
Analyzes, elaborates, and runs cleanly under GHDL (VHDL-2008); confirmed
as part of the full-repo `run.py --compile`. `modules/canny/test/`
has a dedicated IP-level integration testbench (`tb_canny_top.vhd`,
`module_canny.py`) driven by the Python golden model
