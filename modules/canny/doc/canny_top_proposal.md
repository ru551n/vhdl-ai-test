...
Structural top only, per `modules/canny/doc/canny_top_req.md` and
...
`img_width`/`img_height` propagate into `canny_sobel3x3` and
...
`modules/canny/doc/canny_top_req.md` (which is itself kept consistent
...
- Direction fork: `axi_stream.axi_stream_fifo` (hdl-modules, unmodified,
  ...
  Depth: `next_pow2(2*img_width + 16)` — hdl-modules' underlying
  ...
  `2*img_width+2` cycles" sizing guidance up to a legal depth.
- `join`: `axi_stream_join` (`data_width_a=>99, data_width_b=>2`) ->
  `nms`.
- `nms`: `canny_nms`, no generics.
- `threshold`: `canny_threshold` (`thresh_low`, `thresh_high`
  propagated from `canny_top`'s own generics).
- `W4`: `canny_window3x3` (`data_width=>2, user_width=>2`) ->
  `hysteresis`.
...
Single clock domain (`clk`), single synchronous active-high `reset` fed
unchanged to every submodule instance (including the reused
`axi_stream_fifo`, itself stateless w.r.t. `reset` since it has no reset
...
assertion failures) under GHDL** with `img_width=16, img_height=16,
thresh_low=50, thresh_high=100`, both as part of the full-repo
...
Beyond that structural smoke check, `modules/canny/test/tb_canny_top.vhd`
+ `module_canny.py` implement a full IP-level integration test driven
