...
none/weak/strong, per `modules/canny/doc/canny_threshold_req.md`:
`m_axis_tdata = "10"` (strong) if `s_axis_tdata >= thresh_high`; `"01"`
(weak) if `thresh_low <= s_axis_tdata < thresh_high`; else `"00"`
(none). `thresh_low <= thresh_high` is assumed and checked by an
...
Generics: `thresh_low : natural`, `thresh_high : natural`.
...
Single clock, resetless. The reused `common.handshake_pipeline` primitive
(see "Architecture and dataflow" below) exposes no reset port at all
(confirmed against `hdl-modules:modules/common/src/handshake_pipeline.vhd`),
and this module has no other internal registers of its own (purely a
combinational compare function feeding the reused elastic stage), so its
VHDL default initial values already give the correct power-up state
(`output_valid <= '0'` etc. at time zero) — no reset port is needed, per
...
- `classify(magnitude, border, thresh_low, thresh_high)` is a pure
  combinational function: returns `"00"` when `border = '1'`
  (unconditionally, checked first); else `"10"` when
  `to_integer(unsigned(magnitude)) >= thresh_high`; else `"01"` when
  `to_integer(unsigned(magnitude)) >= thresh_low`; else `"00"`.
- The compare result, `s_axis_tuser`, and `s_axis_tlast` are packed into
  a single `lane_width = 5`-bit `input_data` lane (`compare_result &
  s_axis_tuser & s_axis_tlast`, MSB to LSB) fed into one
  `common.handshake_pipeline` instance (`data_width => lane_width`).
...
- `input_ready`/`input_valid`/`output_ready`/`output_valid` connect
  ...
  `m_axis_tvalid`.
- An elaboration-time assertion (`assert thresh_low <= thresh_high
  ...
  via `corvidex-mcp`) enforces the requirement's stated precondition.
...
- `thresh_low`/`thresh_high`: `natural`, compared directly against
...
- `s_axis_tdata = thresh_low - 1` / `thresh_low` / `thresh_high - 1`
  / `thresh_high`: the four boundary values directly exercise the
...
- `thresh_low = thresh_high`: legal per the assertion (`<=`, not
  `<`); no magnitude can land in `"01"` in that configuration (`mag >=
  thresh_high` and `mag >= thresh_low` become the same test), so
  every non-border beat is either `"00"` or `"10"` — not specifically
  exercised by this module's own directed tests (the chosen
  `thresh_low=100`/`thresh_high=200` keep them distinct) but provably
  correct from the `classify` function's structure.
- `thresh_low > thresh_high`: rejected at elaboration by the
...
- Concrete generics for the test DUT: `thresh_low => 100`,
  `thresh_high => 200` (within the 0..2040 magnitude range quoted in
...
- `test_boundary_values`: directed, exactly `thresh_low - 1`,
  `thresh_low`, `thresh_high - 1`, `thresh_high`, non-border,
