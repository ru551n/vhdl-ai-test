...
Entity `canny_gaussian3x3`, architecture `a`.
...
Single clock (`clk`), resetless. The wrapped `common.handshake_pipeline`
instance — the only sequential state this module owns — has no reset
port at all, confirmed against
`hdl-modules:modules/common/src/handshake_pipeline.vhd`. Its internal
state starts at its VHDL default (`wait_for_input_valid`, `output_valid
= '0'`, `input_ready = '1'`) at elaboration/power-up, which is the
same externally-observable "empty, ready" state a runtime reset would
produce, so no reset port is needed on this entity, per
