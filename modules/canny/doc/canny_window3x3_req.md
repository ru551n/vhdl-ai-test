...
raster-scan stream of `img_width x img_height` samples, with full
...
| `s_axis_tuser` | in | std_logic_vector(user_width-1 downto 0) — bit 0 = SOF; bit 1 (only present when `user_width=2`) = border |
| `s_axis_tlast` | in | std_logic — end-of-line (EOL) |
| `m_axis_tvalid` | out | std_logic |
| `m_axis_tready` | in | std_logic |
| `m_axis_tdata` | out | std_logic_vector(9*data_width-1 downto 0) — 3x3 window, packed row-major MSB-to-LSB: `w_tl` at the top bits ... `w_br` at the bottom `data_width` bits |
...
## Clock/reset
Single clock, synchronous active-high reset clears all internal buffers,
...
- Internally realizes 2 row (line) buffers of depth `img_width` (elastic
  ...
  bottom), so the window becomes available once `2*img_width + 2`
...
- `m_axis_tvalid` follows `s_axis_tvalid`'s accepted-beat image exactly
  (1:1 once the pipeline has filled), so exactly `img_width*img_height`
...
- `m_axis_tuser(1)` (border) = this module's own edge-of-frame test
  ...
  row 0, row `img_height-1`, col 0, or col `img_width-1`) **OR**'d with
  ...
  when `user_width=1`). This is a proper dilate of the border ring by
...
- `m_axis_tlast` (EOL) is recomputed from this module's own output-side
  column counter (`col = img_width-1`), not assumed to survive
  windowing unchanged from `s_axis_tlast`.
- `canny_window3x3` recomputes its own row/column counters from the stream
  of *accepted* `s_axis_tlast` pulses (does not trust `img_width` alone
