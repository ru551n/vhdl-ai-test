...
| Generic | Type | Notes |
|---|---|---|
| `thresh_low` | natural | below this: "none" |
| `thresh_high` | natural | at/above this: "strong"; between: "weak" |
...
`m_axis_tdata = "10"` if `s_axis_tdata >= thresh_high`; `"01"` if
`thresh_low <= s_axis_tdata < thresh_high`; else `"00"`.
`thresh_low <= thresh_high` is assumed (an elaboration-time assertion
