...
-- -------------------------------------------------------------------------------------------------
--
-- LOCAL SYNTH-ONLY WORKAROUND COPY -- not the upstream hdl-modules file.
-- The two record<->slv marshalling functions below use a dynamic-width slice
-- (`result(hi downto lo) := ...(user_width - 1 downto 0)`) that trips a real,
-- still-open GHDL synthesis-backend limitation ("cannot extract same variable
-- part for dynamic slice", matches github.com/ghdl/ghdl issue #2658) when
-- run through the ghdl-yosys-plugin import step -- it is NOT a bug in this
-- project's RTL or in tsfpga-mcp. Both affected statements are rewritten
-- below as a fixed-bound (0 to axi_stream_user_sz - 1) bit-by-bit loop with
-- single-bit dynamic indexing, guarded by an `if i < user_width` condition,
-- which avoids the dynamic-width slice entirely while remaining functionally
-- identical to the original for every valid user_width. Verified bit-exact
-- equivalent to the unmodified upstream file by a standalone GHDL simulation
-- (see /tmp/canny_synth_workaround/tb_axi_stream_pkg_equiv.vhd) across
-- data_width in {1, 2, 8, 11, 16, 128} and every user_width in 0 .. 16,
-- with varied data patterns, before being used for any synthesis run.
-- Used ONLY for this synthesis attempt (via tsfpga_synthesize's explicit
-- `libraries` staging), never for simulation/verification of this project,
-- and not committed to hdl-modules -- per the project's ReusableRTL policy,
-- this deviation from "reuse third-party code unmodified" was explicitly
-- signed off by the user for this one synth run.
-- -------------------------------------------------------------------------------------------------
...
package body axi_stream_pkg is
  ...
  begin
    ...
    lo := hi + 1;
    hi := lo + user_width - 1;
    -- Synth-only workaround: fixed-bound bit loop instead of a dynamic-width
    -- slice (`result(hi downto lo) := data.user(user_width - 1 downto 0)`).
    -- Bit-exact equivalent for every valid user_width; see file header.
    for i in 0 to axi_stream_user_sz - 1 loop
      if i < user_width then
        result(lo + i) := data.user(i);
      end if;
    end loop;
  ...
  begin
    ...
    hi := lo + user_width - 1;
    -- Synth-only workaround: fixed-bound bit loop instead of a dynamic-width
    -- slice (`result.user(user_width - 1 downto 0) := data(hi + offset downto lo + offset)`).
    -- Bit-exact equivalent for every valid user_width; see file header.
    for i in 0 to axi_stream_user_sz - 1 loop
      if i < user_width then
        result.user(i) := data(lo + offset + i);
      end if;
    end loop;
