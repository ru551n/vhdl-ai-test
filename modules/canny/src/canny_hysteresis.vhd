...
-- pre-classified window. See modules/canny/doc/canny_hysteresis_req.md
-- and modules/canny/doc/canny_hysteresis_proposal.md.
...
architecture a of canny_hysteresis is
  ...
  constant pipeline_width : positive := 9;
  ...
  begin
    for tap_idx in 0 to 8 loop
      if tap_idx /= 4 and tap(window, tap_idx) = strong then
    ...
    if center = strong then
      result := '1';
    elsif center = weak and any_neighbor_strong = '1' then
      result := '1';
    else
      result := '0';
    end if;
...
begin
  ...
  -- stale/garbage transaction is ever presented while reset = '1'.
  pipeline_input_valid <= s_axis_tvalid and not reset;
  pipeline_output_ready <= m_axis_tready or reset;
  s_axis_tready <= pipeline_input_ready and not reset;
