...
architecture a of canny_sobel3x3 is
  ...
  type window_taps_t is record
    tl, tm, tr, ml, mr, bl, bm, br : unsigned(tap_width - 1 downto 0);
  end record;
  ...
  -- Zero-extend an 8-bit unsigned tap to a grad_width-bit signed value.
  function to_grad(u : unsigned(tap_width - 1 downto 0)) return signed is
  begin
    return signed(resize(u, grad_width));
  end function;

  signal win : window_taps_t;

  signal gx, gy : signed(grad_width - 1 downto 0);
  signal ax, ay : unsigned(mag_width - 1 downto 0);

  signal mag_sum  : unsigned(grad_width - 1 downto 0);
  signal mag_calc : std_logic_vector(mag_width - 1 downto 0);
  signal dir_calc : std_logic_vector(dir_width - 1 downto 0);

  signal mag_pre : std_logic_vector(mag_width - 1 downto 0);
  signal dir_pre : std_logic_vector(dir_width - 1 downto 0);

  signal reg_data_in : std_logic_vector(reg_width - 1 downto 0);

  -- The one shared register both fork outputs are driven from.
  signal reg_valid : std_logic := '0';
  signal reg_data  : std_logic_vector(reg_width - 1 downto 0) := (others => '0');

  signal splitter_input_ready : std_logic;

  signal reg_mag_tdata : std_logic_vector(mag_width - 1 downto 0);
  signal reg_dir_tdata : std_logic_vector(dir_width - 1 downto 0);
  signal reg_tuser     : std_logic_vector(user_width - 1 downto 0);
...
begin
  ...
  ax <= resize(unsigned(abs(gx)), mag_width);
  ay <= resize(unsigned(abs(gy)), mag_width);

  -- Max ax+ay = 1020+1020 = 2040, fits unsigned 11-bit (max 2047) exactly.
  mag_sum  <= resize(ax, grad_width) + resize(ay, grad_width);
  mag_calc <= std_logic_vector(resize(mag_sum, mag_width));
  ...
  -- Hand-written one-cycle elastic register (reset-clearable, unlike
  ...
  reg_mag_tdata <= reg_data(reg_width - 1 downto dir_width + user_width + 1);
  reg_dir_tdata <= reg_data(dir_width + user_width downto user_width + 1);
  reg_tuser     <= reg_data(user_width downto 1);
