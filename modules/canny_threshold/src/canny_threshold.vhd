...
architecture rtl of canny_threshold is
  ...
  signal input_data, output_data : std_logic_vector(c_lane_width - 1 downto 0);

begin

  ------------------------------------------------------------------------------
  assert g_thresh_low <= g_thresh_high
    report "canny_threshold: g_thresh_low (" & natural'image(g_thresh_low) &
      ") must be <= g_thresh_high (" & natural'image(g_thresh_high) & ")"
    severity failure;

  ------------------------------------------------------------------------------
  compare_result <= classify(s_axis_tdata, s_axis_tuser(1), g_thresh_low, g_thresh_high);
